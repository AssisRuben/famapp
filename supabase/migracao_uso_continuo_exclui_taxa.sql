-- ============================================================
-- Migração pra rodar no projeto Supabase REAL — exclui taxa de
-- entrega/frete de vw_clientes_produtos_vendedor.
--
-- Achado: "TAXA DE ENTREGA", "PRODUTO TAXA DE ENTREGA IFOOD",
-- "ENTREGA MANUAL", "TAXA E-DELIVERY" etc. estavam entrando como
-- "produto de uso contínuo" pros clientes (cobrada em toda venda com
-- entrega, não é hábito de recompra de verdade). grupo é
-- inconsistente pra excluir (LINHA GERAL, CONVENIENCIA, USO OU
-- CONSUMO também têm produto de verdade), então filtra por
-- categoria='SERVICOS' + nome contendo entrega/delivery/frete.
-- ============================================================

create or replace view vw_clientes_produtos_vendedor as
with compras as (
  select
    v.codigo_vendedor,
    v.codigo_cliente,
    vi.codigo_produto,
    coalesce(pc.nome, 'Produto ' || vi.codigo_produto) as nome_produto,
    pc.categoria,
    pc.grupo,
    v.id as venda_id,
    v.data_emissao
  from vendas v
  join venda_itens vi on vi.venda_id = v.id
  left join produto_catalogo pc on pc.codigo = vi.codigo_produto
  where v.codigo_vendedor is not null and v.codigo_cliente is not null
    and coalesce(pc.categoria, '') <> 'SERVICOS'
    and coalesce(pc.nome, '') !~* 'entrega|delivery|frete'
),
agregado as (
  select
    codigo_vendedor,
    codigo_cliente,
    codigo_produto,
    nome_produto,
    categoria,
    grupo,
    count(distinct venda_id) as qtd_compras,
    max(data_emissao) as ultima_compra,
    (max(data_emissao) - min(data_emissao))::numeric / nullif(count(distinct venda_id) - 1, 0) as intervalo_medio_dias
  from compras
  group by codigo_vendedor, codigo_cliente, codigo_produto, nome_produto, categoria, grupo
)
select
  codigo_vendedor,
  codigo_cliente,
  codigo_produto,
  nome_produto,
  categoria,
  grupo,
  qtd_compras,
  ultima_compra,
  round(intervalo_medio_dias, 1) as intervalo_medio_dias,
  (current_date - ultima_compra) as dias_desde_ultima_compra,
  (qtd_compras >= 2) as recorrente,
  (
    qtd_compras >= 2
    and intervalo_medio_dias is not null
    and (current_date - ultima_compra) > intervalo_medio_dias * 1.3
  ) as atrasado
from agregado;

-- ---------- VERIFICAÇÃO (opcional, só leitura) ----------
-- Confirma que os códigos de taxa/entrega sumiram da view:
-- select * from vw_clientes_produtos_vendedor
-- where codigo_produto in (18793, 21347, 24766, 1961, 24877, 25463);
-- (deve retornar 0 linhas)
